 ...
 *               (j) create graphics pipeline
 *               (k) create the framebuffer
 *               (l) create the command pool
 *               (m) create command buffers
 *               ( ) createUniformBuffers();
 *               ( ) createDescriptorPools();
 *               ( ) createDescriptorSets();
 *               (n) record commands to render the scene
 *               (o) create synchronization objects to use along the graphics pipeline
 *            If any of these steps fails, it will throw a runtime exception and the program will terminate.
 * 
 *            in addition to the above vulkan creation steps we also set up the vertex buffers, the index buffer and 
 *            the inital model-view-projection matrix.
 *
 * parameters: void
 *
...
int vkContext::initContext()
{
  try
  {
    ...
    m_uboVP.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

    m_uboVP.proj[1][1] *= -1;               // vulkan reverses the direction of the y-axis

    // create two meshs, vertex data
    ...
    mesh fstmesh = mesh(m_device.physical, m_device.logical, m_graphicsQueue, m_graphicsCommandPool, &meshVertices, &meshIndices);
    mesh scdmesh = mesh(m_device.physical, m_device.logical, m_graphicsQueue, m_graphicsCommandPool, &meshVertices2, &meshIndices);

    m_meshList.push_back(fstmesh);
    m_meshList.push_back(scdmesh);
...
void vkContext::cleanupContext()
{
  ...
  for (size_t i = 0; i < m_swapChainImages.size(); i++)
  {
    ...
    vkDestroyBuffer(m_device.logical, m_modelDUniformBuffer[i], nullptr);
    vkFreeMemory(m_device.logical, m_modelDUniformBufferMemory[i], nullptr);
  }
...
void vkContext::createDescriptorSetLayout()
{
  ...
  {
    std::cerr << "[+] sucessfully created descriptor set layout" << std::endl;
  }
}
...
void vkContext::createUniformBuffers()
{
  ...
  VkDeviceSize vpBufferSize = m_modelUniformAlignment*MAX_OBJECTS;
  ...
  for (size_t i = 0; i < m_swapChainImages.size(); i++)
  {
    ...
    createBuffer(m_device.physical, m_device.logical, modelBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &m_modelDUniformBuffer[i], &m_modelDUniformBufferMemory[i]);
  }
}
...
void vkContext::createDescriptorPool()
{
  ...
  modelPoolSize.descriptorCount = static_cast<uint32_t>(m_modelDUniformBuffer.size());
  ...
  {
    std::cerr << "[+] successfully created a descriptor pool" << std::endl;
  }
}
...
void vkContext::createDescriptorSets()
{
  ...
  for (size_t i = 0; i < m_swapChainImages.size(); i++)
  {
    ...
    modelBufferInfo.buffer = m_modelDUniformBuffer[i];
...
void vkContext::updateUniformBuffers(uint32_t imageIndex)
{
  ...
  vkMapMemory(m_device.logical, m_modelDUniformBufferMemory[imageIndex], 0, m_modelUniformAlignment * m_meshList.size(), 0, &data);
  memcpy(data, m_modelTransferSpace, m_modelUniformAlignment * m_meshList.size());
  vkUnmapMemory(m_device.logical, m_modelDUniformBufferMemory[imageIndex]);
}
 ...
 *           : modified Apr2024 to support descriptor sets
************************************************************************************************************************/
 ...
 *
 * abstract  : This function allocates aligned memory space for use in transfering data from teh CPU to the GPU for use
 *             in dynamic uniform descriptor sets
 *
 * parameters:
 *
 * returns   :
 ...
 * written   : Apr 2024 (GKHuber)
************************************************************************************************************************/
void vkContext::allocateDynamicBufferTransferSpace()
{
