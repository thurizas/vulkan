...
int vkContext::initContext()
{
  try
  {
    ...
    mvp.proj = glm::perspective(glm::radians(45.0f), (float)m_swapChainExtent.width / (float)m_swapChainExtent.height, 0.1f, 100.0f);
    mvp.view = glm::lookAt(glm::vec3(0.0f, 0.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
    mvp.model = glm::mat4(1.0f);

    mvp.proj[1][1] *= -1;               // vulkan reverses the direction of the y-axis
...
void vkContext::updateModel(glm::mat4 newModel)
{
  mvp.model = newModel;
}
...
void vkContext::cleanupContext()
{
  ...
  {
    vkDestroyBuffer(m_device.logical, m_uniformBuffer[i], nullptr);
    vkFreeMemory(m_device.logical, m_uniformBufferMemory[i], nullptr);
  }
...
void vkContext::createUniformBuffers()
{
  ...
  {
    createBuffer(m_device.physical, m_device.logical, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
void vkContext::createDescriptorSets()
{
  ...
  for (size_t i = 0; i < m_uniformBuffer.size(); i++)
  {
    ...
    vkUpdateDescriptorSets(m_device.logical, 1, &mvpSetWrite, 0, nullptr);
  }
}
...
{
  void* data;
  vkMapMemory(m_device.logical, m_uniformBufferMemory[imageIndex], 0, sizeof(MVP), 0, &data);
  memcpy(data, &mvp, sizeof(MVP));
  vkUnmapMemory(m_device.logical, m_uniformBufferMemory[imageIndex]);
}
