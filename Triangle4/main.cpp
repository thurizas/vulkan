...
int main(int argc, char** argv)
{
  ...
  if (EXIT_SUCCESS == ctx.initContext())
  {
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      firstModel = glm::translate(firstModel, glm::vec3(0.0f, 0.0f, -2.5f));                  // red object 
      firstModel = glm::rotate(firstModel, glm::radians(angle), glm::vec3(0.0f, 0.0f, 1.0f));

      secondModel = glm::translate(secondModel, glm::vec3(0.0f, 0.0f, -3.0f));                 // blue object 
      secondModel = glm::rotate(secondModel, glm::radians(-angle*10.0f), glm::vec3(0.0f, 0.0f, 1.0f));

      ctx.updateModel(0, firstModel);
      ctx.updateModel(1, secondModel);
